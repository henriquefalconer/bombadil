...
- Header stripping decisions should account for resource type. A header that is safe to strip from a Script response (e.g., CSP, which only contains script-hash directives for that resource) may not be safe to strip from a Document response (where the same header governs the entire page security policy). When the same `FulfillRequestParams` code path serves multiple resource types, either use separate denyists or verify that each entry is valid for all resource types that pass through it.
